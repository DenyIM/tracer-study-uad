...
<body>
        ...
        <div class="container">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div class="text-end">
                    ...
                    <p class="text-muted mb-0 small">{{ auth()->user()->alumni->study_program }}</p>
                </div>
            </div>
        </div>
    </header>
                ...
                <div class="col-lg-8" data-aos="fade-up">
                    <div class="completion-card">
                        ...
                        <div class="category-badge mb-3">
                            <i class="fas {{ $statusQuestionnaire->category->icon ?? 'fa-folder' }} me-2"></i>
                            {{ $statusQuestionnaire->category->name }}
                        </div>

                        <p class="lead mb-4">
                            Terima kasih atas kontribusi Anda dalam mengisi semua kuesioner Tracer Study UAD.
                            Data yang Anda berikan sangat berharga untuk pengembangan almamater.
                        </p>

                        <div class="completion-date">
                            <i class="fas fa-calendar-check me-1"></i>
                            Diselesaikan:
                            {{ $statusQuestionnaire->completed_at ? $statusQuestionnaire->completed_at->format('d F Y') : now()->format('d F Y') }}
                        </div>

                        <div class="stats-display">
                            <div class="stat-item" data-aos="fade-right" data-aos-delay="100">
                                <div class="stat-value points" id="pointsValue">{{ $totalPoints }}</div>
    ...
    <script>
        ...
        // Jalankan animasi counter untuk points
        const totalPoints = {{ $totalPoints }};
        setTimeout(() => {
            animateCounter('pointsValue', totalPoints);
        }, 500);

        // Simpan status penyelesaian
        localStorage.setItem('all_questionnaires_completed', 'true');
        localStorage.setItem('completion_date', new Date().toISOString());
        localStorage.setItem('completed_category', '{{ $statusQuestionnaire->category->name }}');
        ...
        if (!hasAchievement) {
            achievements.push({
                ...
                title: 'Kompletor Kuesioner - {{ $statusQuestionnaire->category->name }}',
                description: 'Telah menyelesaikan semua kuesioner Tracer Study untuk kategori {{ $statusQuestionnaire->category->name }}',
                date: new Date().toISOString(),
                points: totalPoints,
                category: '{{ $statusQuestionnaire->category->name }}'
            });
        ...
        setTimeout(() => {
            showToast(
                `Selamat! Anda telah menyelesaikan kuesioner kategori {{ $statusQuestionnaire->category->name }}.`,
                'success');
        }, 1000);
    </script>
</body>
